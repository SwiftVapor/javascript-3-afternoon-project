...
//Code Here
let postTaxPrices  = prices.map((ele, ind, arr) => {
  return ele * 1.07
});
...
//Code Here
let myStrongest = monstersInYourPocket.filter((ele, ind, array)=>{
  return ele.CP > 200;
});
...
*/
//code here
let orderTotals = orders.map((ele, ind, arr) => {
...
let sortBob = purchases.filter ((ele, ind, arr) => {
  return ele.owner === "Bob";
 });
 
 
 let bobsTotalRaw = sortBob.map ((ele, ind, arr) => {
     return ele.price
 });
 
 
 let bobsTotal = bobsTotalRaw.reduce ((acc, ele, ind, arr) => {
     return acc + ele
 });
